...
\begin{enumerate}[leftmargin=1.5cm]
    ...
    \item La pasarela de borde basada en Raspberry Pi 4 con OTBR + ThingsBoard Edge (transporte LwM2M integrado).
...
\noindent\textbf{Resultado principal:} La configuración V3 alcanzó una tasa de éxito del \textbf{97,3\,\%} (214/220 lecturas exitosas) en la lectura de recursos LwM2M a través de la cadena completa TB Edge REST API $\rightarrow$ CoAP $\rightarrow$ Thread mesh $\rightarrow$ ESP32-C6, con latencia mediana de \SI{33}{\milli\second} y recuperación de lecturas lentas en la banda de 5--10 segundos gracias a la sinergia entre retransmisiones rápidas del motor CoAP y ventana extendida de la API ThingsBoard Edge.
...
\addlinespace
\textbf{2} & Pasarela de Borde & Raspberry Pi 4 ejecutando OpenThread Border Router (OTBR) con ThingsBoard Edge (transporte LwM2M integrado), PostgreSQL, Grafana; enlace HaLow STA (IEEE 802.11ah) hacia Nivel 3 \\
\addlinespace
...
Cada nodo se registra en el servidor LwM2M de ThingsBoard Edge con un nombre de endpoint único derivado de los últimos 2 bytes de la dirección MAC del SoC:

\begin{center}
\texttt{ami-esp32c6-\textit{XXXX}}
\end{center}

\noindent Ejemplo: \texttt{ami-esp32c6-25c0}. La comunicación LwM2M se realiza sobre CoAP/UDP hacia la dirección mesh-local del OTBR donde ejecuta ThingsBoard Edge:
...
Se diseñaron cinco objetos LwM2M custom para exponer diagnósticos detallados de la red Thread, definidos en archivos XML estándar OMA:
...
\noindent Los modelos XML se encuentran en \texttt{models/\{10483,10484,10485,10486,33000\}.xml} y se cargan en ThingsBoard Edge para habilitar la decodificación correcta de recursos custom.
...
\begin{enumerate}[leftmargin=1.5cm]
    ...
    \item \textbf{Servidor LwM2M:} ThingsBoard Edge con transporte LwM2M integrado, exponiendo API REST en puerto 8090 y CoAP en puerto 5683.
    \item \textbf{Motor de reglas y persistencia:} ThingsBoard Edge con PostgreSQL para almacenamiento time-series local.
\end{enumerate}

\subsection{Servidor LwM2M (ThingsBoard Edge)}

\textbf{ThingsBoard Edge} integra un transporte LwM2M nativo basado en Eclipse Californium. En la arquitectura del piloto:

\begin{table}[H]
\centering
\caption{Configuración del servidor LwM2M en el gateway}
\label{tab:lwm2m-server}
\begin{tabularx}{\textwidth}{l X}
\toprule
\textbf{Parámetro} & \textbf{Valor} \\
\midrule
Versión & ThingsBoard Edge 4.2.1EDGE \\
Contenedor Docker & \texttt{tb-edge} en OTBR (\SI{192.168.1.111}{}) \\
Puerto CoAP & 5683 (UDP, no seguro --- modo NoSec para piloto) \\
Puerto API REST & 8090 (API REST + interfaz web de gestión) \\
Modelos LwM2M custom & 10483, 10484, 10485, 10486, 33000 \\
...
\textbf{Parámetro} & \textbf{Valor} \\
\midrule
Versión & ThingsBoard Edge 4.2.1EDGE sobre Californium \\
Contenedor Docker & \texttt{tb-edge} en OTBR (\SI{192.168.1.111}{}) \\
Puerto CoAP & 5683 (UDP, no seguro --- modo NoSec para piloto) \\
...
Un hallazgo crítico de esta investigación fue la importancia de la configuración del motor CoAP \textbf{Californium} que subyace al transporte LwM2M de ThingsBoard Edge. Los parámetros de retransmisión impactan directamente en la tasa de éxito de lecturas LwM2M:
...
\begin{enumerate}[leftmargin=1.5cm]
    ...
    \item \textbf{Registro LwM2M:} El cliente envía \texttt{Register} al servidor LwM2M de ThingsBoard Edge con lista de objetos soportados.
    \item \textbf{TB Edge REST API} recibe solicitud de lectura HTTP (e.g., \texttt{GET /api/plugins/telemetry/DEVICE/\{id\}/values/timeseries}).
    ...
    \item \textbf{ThingsBoard Edge} devuelve el valor al cliente REST en formato JSON.
\end{enumerate}
...
Se realizaron tres campañas de pruebas sistemáticas para optimizar el rendimiento de lectura de recursos LwM2M a través de la API REST del servidor. Cada campaña utilizó un script PowerShell automatizado que lee 22 recursos distintos en 10 rondas (\textbf{220 lecturas totales por prueba}).
...
\midrule
V1 & \SI{2500}{\milli\second} & Defecto servidor & Línea base con Californium optimizado \\
V2 & \SI{5000}{\milli\second} (defecto RFC) & Defecto servidor & Calif.\ más lento, sin API timeout \\
...
Con \texttt{ACK\_TIMEOUT=2500ms} en Californium, las retransmisiones CoAP son agresivas: primer retry a \SI{2.5}{\second}, segundo a \SI{6.3}{\second}. Esto permite recuperar paquetes perdidos dentro de la ventana de timeout por defecto de la API del servidor ($\sim$\SI{5}{\second}). La mediana de \SI{27}{\milli\second} indica que la mayoría de lecturas son rápidas, pero 12 lecturas agotaron el timeout.
...
\begin{itemize}[leftmargin=1.2cm]
    ...
    \item \textbf{API del servidor \texttt{?timeout=10}:} Ventana de espera de \SI{10}{\second} en la API REST.
...
El factor más influyente en la fiabilidad de lecturas LwM2M no es la calidad del enlace radio ni la capacidad del nodo, sino la \textbf{interacción entre los timeouts del motor CoAP (Californium) y la ventana de espera de la API REST del servidor LwM2M}. Un \texttt{ACK\_TIMEOUT} demasiado conservador (RFC~7252: \SI{5}{\second}) causa que la primera retransmisión llegue justo cuando la API ya ha abortado la espera.

\begin{center}
\fbox{
\begin{minipage}{0.85\textwidth}
\textbf{Recomendación:} Para redes 6LoWPAN/Thread con ThingsBoard Edge, configurar Californium con \texttt{ACK\_TIMEOUT=2500ms} y la API con \texttt{?timeout=10} o superior. Esto proporciona $\geq$3 oportunidades de retransmisión dentro de la ventana de la API.
\end{minipage}
}
\end{center}
...
Un bug sutil en PowerShell 5.1 causó que el parámetro \texttt{?timeout=10} no se enviara correctamente al servidor LwM2M. La expresión:
...
% =============================================================================
\section{Benchmark de Intervalos LwM2M Observe}
\label{sec:benchmark-observe}
% =============================================================================

Para determinar la configuración óptima de las ventanas de observación LwM2M (\texttt{pmin}/\texttt{pmax}),
se ejecutaron campañas de benchmark automatizadas con 3 escenarios de intervalos Observe
sobre el pipeline completo RS-485$\rightarrow$Thread$\rightarrow$TB~Edge.

\begin{table}[H]
\centering
\caption{Resultados del benchmark LwM2M Observe — Firmware v0.15.1 (300\,s por escenario)}
\label{tab:lwm2m-benchmark}
\begin{tabular}{l l r r r r}
\toprule
\textbf{Escenario} & \textbf{pmin/pmax} & \textbf{Msgs} & \textbf{Msgs/s} & \textbf{IAT avg (s)} & \textbf{Keys} \\
\midrule
Baseline (Producción) & 15/30 & 47 & 0,261 & 10,07 & 16/16 \\
Agresivo (1\,s) & 1/1 & 119 & 0,661 & 4,23 & 16/16 \\
Medio (5\,s) & 5/5 & 29 & 0,161 & 18,97 & 16/16 \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{¿Por qué se eliminó el escenario Relajado (10\,s)?}
Con \texttt{pmax=10\,s} y un intervalo de polling DLMS de \SI{15}{\second},
las observaciones expiran (\texttt{pmax}) antes de que el firmware genere la primera notificación.
Resultado: \textbf{0~mensajes} en \SI{300}{\second}. La condición necesaria es $\texttt{pmax} \geq T_{\text{DLMS\_poll}}$.

% --- Análisis de Cuellos de Botella ---
\subsection{Análisis de Cuellos de Botella por Capa}
\label{subsec:bottleneck}

El pipeline de datos atraviesa 4 capas, cada una con su propio cuello de botella.
La capa más lenta determina el throughput máximo de extremo a extremo:

\begin{table}[H]
\centering
\caption{Pipeline de datos AMI: cuellos de botella por capa}
\label{tab:bottleneck}
\begin{tabularx}{\textwidth}{c l r r X}
\toprule
\textbf{Capa} & \textbf{Componente} & \textbf{Latencia} & \textbf{Cadencia} & \textbf{Cuello de botella} \\
\midrule
L1 & RS-485 DLMS/COSEM & 2--3\,s/ciclo & 15\,s & Lectura secuencial de 17 registros OBIS a 9600\,bps.
     El poll de \SI{15}{\second} es el \textbf{factor dominante} del sistema. \\
\addlinespace
L2 & Firmware (Threshold) & $<$\SI{1}{\milli\second} & Evento & Compara lectura vs.\ umbral; solo notifica
     si $|\Delta| > \epsilon$. Latencia despreciable. \\
\addlinespace
L3 & Thread 802.15.4 & 26\,ms/msg & Bajo demanda & Canal \SI{250}{\kilo\bit\per\second} compartido, $\sim$\SI{62}{\byte}/notify CoAP.
     Utilización $<$1\,\% con 1 nodo. \\
\addlinespace
L4 & TB Edge Californium & --- & pmin/pmax & Entrega notificación al cliente solo si
     $t \geq \texttt{pmin}$; descarta si $t > \texttt{pmax}$ sin nuevo dato. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}[leftmargin=1.2cm]
  \item L1 (DLMS \SI{15}{\second} poll) \textbf{domina} el pipeline: ningún \texttt{pmin}$<$\SI{15}{\second}
        puede generar datos más rápido que el poll del bus RS-485.
  \item El escenario \textbf{Agresivo (1\,s)} obtiene más mensajes porque captura
        sub-notificaciones por umbral dentro de un mismo ciclo DLMS, pero no mejora
        la resolución temporal real del medidor.
  \item El \textbf{Baseline (pmin=15/pmax=30)} es óptimo porque:
    \begin{enumerate}
      \item Se alinea con $T_{\text{DLMS\_poll}}=15$\,s, evitando observaciones vacías.
      \item Minimiza tráfico CoAP innecesario ($\times$2,5 menos que Agresivo).
      \item Maximiza escalabilidad de la red Thread (más nodos por BR).
      \item \texttt{pmax=30\,s} ($2\times T_{\text{poll}}$) da margen ante jitter DLMS.
    \end{enumerate}
\end{itemize}
...
--- & \texttt{aa34561} & Optimización V3: API \texttt{?timeout=10} + corrección PS 5.1 \\
\bottomrule
\end{tabularx}
\end{table}
...
\texttt{models/*.xml} & Definiciones XML de objetos custom LwM2M \\
...
\begin{enumerate}[leftmargin=1.5cm]
    ...
    \item \textbf{Optimización CoAP/Californium validada:} La configuración \texttt{ACK\_TIMEOUT=2500ms} (Californium) + \texttt{?timeout=10} (API del servidor) alcanzó 97,3\,\% de éxito vs.\ 90,9\,\% con parámetros RFC por defecto, una mejora de +6,4 puntos porcentuales.
